...
% By previous observation, the 50 first groups are choosen (Group 1 to 
% group 50). These groups represent fashion items. The final subset is 
% composed of 27 507 images.

MAXGROUP = 50;
nameOfFolder = 'trainSetWithBlackWhiteBordersCropped';
if not(isfolder(nameOfFolder))
    mkdir(nameOfFolder)
end
...
for i=1:MAXGROUP
    ...
    for j=1:height(imagesSelected)
        ...
        imCropped = whiteCrop(im);
        file = fullfile(nameOfFolder, string(imagesSelected{j,1}));
        imwrite(imCropped, file);
    end
end
